...
class RepliesModel with ChangeNotifier {
  ...
  RepliesModel(BuildContext context, int? commentId, UserProfile? userdata,
  ...
  Future<void> fetchComments() async {
    try {
      final response = await http.post(Uri.parse(loadreplies),
          body: jsonEncode({
  ...
  Future<void> constructComment(String content) async {
    try {
      ...
      print(data.toString());
      final response = await http.post(Uri.parse(replycomment),
  ...
  Future<void> deleteComment(int? commentId, int position) async {
    ...
    try {
      ...
      print(data.toString());
      final response = await http.post(Uri.parse(deletereply),
          body: jsonEncode({"data": data}));
      if (response.statusCode == 200) {
        // If the server did return a 200 OK response,
        // then parse the JSON.
  ...
  Future<void> editComment(int? id, String content, int position) async {
    ...
    try {
      ...
      print(data.toString());
      final response = await http.post(Uri.parse(editreply),
          body: jsonEncode({"data": data}));
      if (response.statusCode == 200) {
        // If the server did return a 200 OK response,
        // then parse the JSON.
  ...
  Future<void> reportComment(int commentId, int position, String reason) async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        String? _status = res["status"];
        if (_status == "ok") {
          totalCommentsReply = totalCommentsReply! - 1;
          Navigator.of(_context).pop();
          items.removeAt(position);
          notifyListeners();
        } else {
